...
// Step labels used in the contact stepper
const STEPS = [
  { title: "Welcome" },
  { title: "Your Details" },
  { title: "Your Message" },
...
export default function ContactStepper({ steps = STEPS }) {
  ...
  const handleSubmit = async (event) => {
    ...
    // TODO: replace this with your real third‑party endpoint,
    // for example a Formspree URL like:
    ...
    try {
      const response = await fetch(THIRD_PARTY_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          email,
          message,
          to: "prajwalhulamani5111999@gmail.com",
        }),
      });

      if (!response.ok) {
        console.error("Contact form third‑party API error:", response.status);
      }
    } catch (error) {
      console.error("Contact form third‑party API exception:", error);
    }

    // Regardless of API result, move to final step so the user sees confirmation.
    setActiveIndex(3);
  };
        ...
        {activeIndex === 0 && (
          <div className="step-animated">
            <h2 className="text-xl md:text-2xl font-semibold text-violet-400">
              If you’ve landed here, you’re already one step closer to a meaningful collaboration.
            </h2>
        ...
        {activeIndex === 2 && (
          <div className="step-animated">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 mt-2">
              ...
              <button
                type="submit"
                className="w-full rounded-full bg-sky-500 text-slate-900 font-semibold py-2 text-sm md:text-base hover:bg-sky-400 transition-colors"
              >
                Send Message 🚀
              </button>
            </form>
